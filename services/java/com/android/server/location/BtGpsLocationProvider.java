...
public class BtGpsLocationProvider implements LocationProviderInterface {
    ...
    // Bitfield of pending messages to our Handler used only for
    // messages that cannot have multiple instances queued
    ...
    /**
     * Listen for BT changes.
     * If Bluetooth is turned off, disable GPS services
     */
    ...
    // This class handles the actual Bluetooth connection and data xfer
    private final BtGpsService btsvc;

    // BT Location provider,
    // uses the same method signature as the original GPS location provider
                ...
                if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                    ...
                    switch (state) {
                        case BluetoothAdapter.STATE_ON:
                            if (D) Log.i(TAG, "Bluetooth turned on -> notify services?");
                            break;
                        case BluetoothAdapter.STATE_TURNING_OFF:
                            if (btsvc.getServiceState() != BtGpsService.STATE_NONE) {
                                if (D) Log.i(TAG, "Bluetooth turned off -> stopping services");
                                btsvc.stop();
                            }
                            break;
                    }
        ...
        public void run() {
            ...
            mHandler = new Handler() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    // Release wakelock if no messages are pending
    ...
    /**
     * Enables Bluetooth GPS provider
     */
    private synchronized void handleEnable() {
        ...
        if (!mAdapter.isEnabled()) {
            ...
            if (state == BluetoothAdapter.STATE_OFF) {
                if (D) Log.d(TAG, "Bluetooth not available. Enable and wait for it...");
                mAdapter.enable();
            }
            ...
            while (true) {
                try {
                    ...
                    if (state == BluetoothAdapter.STATE_ON) {
                        ...
                        if (D) Log.d(TAG, "Bluetooth not available yet. waiting for another 400ms");
                        Thread.sleep(400);
                    } else {
                        if (D) Log.d(TAG, "Bluetooth got disabled or interrupted by other source");
                        return;
                    }
                } catch (InterruptedException e) {
    ...
    private void reportSvStatus(int svCount, int mSvs[], float mSnrs[],
        ...
        // Return the number of sets used in fix instead of total
    ...
    private final INetInitiatedListener mNetInitiatedListener = new INetInitiatedListener.Stub() {
        ...
        public boolean sendNiResponse(int notificationId, int userResponse) {
            // TODO: Don't know that should be here
            return true;
        }
    };
